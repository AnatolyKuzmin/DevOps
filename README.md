...
5. Playbook. YAML-сценарий с задачами для выполнения
6. Roles. Наборы предопределенных задач и конфигурация для повторного использования
...
Ключевые компоненты:
1. Модули - Это готовые «кирпичики» для выполнения задач. apt — установка пакетов в Debian/Ubuntu. copy — копирование файлов на удаленные хосты. service — управление сервисами.
2. Идемпотентность - Ansible гарантирует, что повторный запуск задачи не изменит систему, если она уже в нужном состоянии.
3. Переменные - Позволяют делать плейбуки гибкими.
4. Обработчики (Handlers) - Запускаются только при изменении состояния. 
